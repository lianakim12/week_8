...
df = pd.get_dummies(data=df, columns=["Type", "Category"], drop_first=True,
                    prefix=["Ty", "Cat"]) # drop_first=True removes the base level
df.head()
...
print(f"With Intercept: Coefficient = {model_with.coef_[0]:.4f}, Intercept = {model_with.intercept_:.4f}")
print(f"Without Intercept: Coefficient = {model_without.coef_[0]:.4f}, Intercept = {model_without.intercept_:.4f}")
...
mlr_features = corr_with_target[5:11].index.tolist() # exclude target variable
